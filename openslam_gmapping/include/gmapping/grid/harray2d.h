...
class HierarchicalArray2D: public Array2D<autoptr< Array2D<Cell> > >{
	...
	protected:
		...
		int m_patchSize;		//patch的实际大小 m_patchSize=1<<m_patchMagnitude;
};

/*
@desc 构造函数，创建一个Array2D,大小为xsize*ysize 等级为patchMagnitude
xsize和ysize是在patchMagnitude=0时候的大小。
如果等级上升的话，创建的大小要想要的缩小。
patchMagnitude每增大1，则Array2D的面积要缩小一半。
类似于金字塔
*/
template <class Cell>
